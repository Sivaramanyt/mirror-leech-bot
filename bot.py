...
# ✅ FIXED: URL handler with correct filter syntax (NO ~filters.command)
@app.on_message(filters.text & filters.private)
async def handle_url(client: Client, message: Message):
    """Handle direct URL messages with aiofiles support - FIXED FILTER"""
    try:
        # ✅ CRITICAL: Skip commands manually (no ~filters.command)
...
async def main():
    ...
    try:
        logger.info("🚀 Starting FIXED Terabox Bot with Pyrogram + aiofiles...")
        ...
        logger.info("⚡ Filter syntax FIXED - No more TypeError!")
        ...
        # ✅ CRITICAL: Start bot with all fixes
        try:
            await app.start()
            me = await app.get_me()
            ...
            logger.info("⚡ Filter syntax fixed - No more TypeErrors!")
            logger.info("🔥 Bot will respond to ALL messages and commands!")
            logger.info("🎯 Production ready with complete functionality!")
